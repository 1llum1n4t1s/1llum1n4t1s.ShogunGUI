...
- **アプリルート**: 以下の 3 つの処理だけで使用する。**(1) Node.js のインストール**（`%LocalAppData%\Shogun.Avalonia\nodejs`）、**(2) Claude Code のインストール**（同 `npm`）、**(3) log4net のログ出力**（exe の BaseDirectory）。queue/config の基準とは別。
- **queue/config の基準＝ドキュメントルート**: config/settings.yaml の `document_root`（ドキュメントルート）の直下に config/, queue/, instructions/ 等がある。RUNNER_CWD・キュー・instructions はここを参照。未設定時は config の親を自動参照する。
- **Claude Code 環境**: 設定画面の「Claude Code 環境」で、**アプリ内に Node.js と Claude Code CLI をインストール**する（環境を汚さない。アプリルート `%LocalAppData%\Shogun.Avalonia` に配置）。「Node.js をインストール」→「Claude Code をインストール」の順で実行する。
